...
object AccountService:
    ...
    def findAll(): List[GetAccount] = AccountRepository.findAll().map(account => GetAccount(account.id, account.name, BalanceRepository.findByAccountId(account.id)))

    def updateBalanceValue(accountId: String, balanceType: BalanceType, value: Float): Unit =
        val balance = BalanceRepository.findByAccountIdAndBalanceType(accountId, balanceType).get
        BalanceRepository.update(balance.updateValue(value))


    private def generateBalances(account: Account): Array[Balance] = BalanceType.values.map(Balance(account.id, _, 0))